/// 哈气站 MD3 主题：默认白色底 + 活力粉；支持传入外部配色方案
/// （系统动态取色 / 用户自定义种子色）复用同一套组件样式。
library;
...
/// 应用内置默认配色（活力粉），未开启动态取色/自定义时使用。
ColorScheme defaultLightScheme() => ColorScheme.light(
      primary: _primary,
      ...
      secondaryContainer: _borderPink,
    );

/// 应用内置默认深色配色。
ColorScheme defaultDarkScheme() => ColorScheme.dark(
      ...
      secondaryContainer: const Color(0xFF3B2A33),
    );

/// 由种子色生成明/暗两套配色（自定义颜色模式）。
(ColorScheme, ColorScheme) schemesFromSeed(Color seed) => (
      ColorScheme.fromSeed(seedColor: seed),
      ColorScheme.fromSeed(seedColor: seed, brightness: Brightness.dark),
    );

ThemeData buildLightTheme([ColorScheme? scheme]) =>
    _buildTheme(scheme ?? defaultLightScheme(), dark: false);

ThemeData buildDarkTheme([ColorScheme? scheme]) =>
    _buildTheme(scheme ?? defaultDarkScheme(), dark: true);
...
ThemeData _buildTheme(ColorScheme scheme, {required bool dark}) {
  return ThemeData(
    ...
    navigationBarTheme: NavigationBarThemeData(
      backgroundColor: dark ? scheme.surfaceContainer : scheme.surfaceContainerLow,
